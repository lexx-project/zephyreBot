...
export default {
  name: "cekganteng",
  description: "Mengecek seberapa ganteng seseorang secara acak.",
  usage: `${config.prefix}cekganteng <nama/tag>`,
  category: "fun",
  aliases: ["gantengcek"],
  cooldown: 5,
  ...
  async execute(sock, m, args) {
    ...
    try {
      ...
      const adjective = ["ganteng", "jelek"][Math.floor(Math.random() * 2)];
      ...
      const allAnswers = [
        ...
        "Jelek",
        "Jelek Amat 🤢",
        "Gila, gantengnya kebangetan!",
      ];

      const finalAnswer =
        allAnswers[Math.floor(Math.random() * allAnswers.length)];

      const replyText = `*Pertanyaan:* Cek Ganteng ${targetText}\n*Jawaban:* ${finalAnswer}`;
      ...
      logger.info(`[CEKGANTENG] Command executed by ${senderNumber}`);
    } catch (error) {
      logger.error(`[CEKGANTENG] Error: ${error.message}`);
      await sock.sendMessage(
        m.key.remoteJid,
        {
