...
function roleGenerator(from, data) {
  ...
  const playerCount = session.player.length;
  const roleCounts = roleAmount(playerCount);
  ...
  // Shuffle roles and players for truly random assignment
  const shuffledRoles = roleShuffle(roles);
  const shuffledPlayers = roleShuffle(session.player);
