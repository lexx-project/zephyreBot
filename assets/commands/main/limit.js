  ...
  async execute(sock, message, args) {
    try {
      ...
      if (user.limit <= 5) {
        ...
        if (!isOwner(userId)) {
          responseText += `💰 *Beli Limit:* Gunakan ${config.prefix}buylimit untuk membeli limit tambahan (15 saldo per limit)\n`;
          responseText += `💡 *Tip:* Upgrade ke Premium untuk limit 500/hari`;
        }
      } else if (!isOwner(userId)) {
        responseText += `💰 *Info:* Gunakan ${config.prefix}buylimit untuk membeli limit tambahan (15 saldo per limit)`;
      }
